...
import {models as inferenceModels} from './catalog.js?v=22';
...
export function normalizeTraining(raw={}){
 ...
 for(const k of ['buySharding','rentSharding','recipeConfirmed'])s[k]=s[k]?1:0;
 return s;
}
...
export function calculateTraining(raw){
 ...
 function path(kind){
  ...
  if(h.gpus>1&&!s[kind+'Sharding'])issues.push('Confirm a training recipe that shards this model across these GPUs. Aggregate VRAM alone is insufficient.');
  ...
  if(issues.length)return {kind,h,requiredGB,availableGB,minimumGPUs,issues,ready:false};
  ...
  return {kind,h,requiredGB,availableGB,minimumGPUs,issues,ready:true,hours,cadenceFits:hours[1]*s.runsPerMonth<=730,gpuHours:hours.map(t=>t*h.gpus),operating,perRun,campaign,capital:kind==='buy'?capital:0};
 }
