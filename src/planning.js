...
export function planning(raw={}){
 ...
 if(s.autoHardware){const candidates=hardware.filter(h=>verifiedConfiguration('inference',s,'buy',h.id)&&(h.gpuCeiling??h.memory)*policy.localUsable>=required);s.hardware=(candidates.sort((a,b)=>(a.price??prices[a.id])-(b.price??prices[b.id]))[0]??{id:s.hardware}).id;}
 const h=hardware.find(h=>h.id===s.hardware);
 if(s.autoRental){s.rental=(rentals.filter(r=>verifiedConfiguration('inference',s,'rent',r.id)&&r.memory*policy.rentalUsable>=required).sort((a,b)=>a.hourly-b.hourly)[0]??{id:s.rental}).id;}
