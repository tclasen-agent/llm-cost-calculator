...
### Additional post-training methods

The method dropdown preserves old LoRA/QLoRA links. Continued pretraining, DPO, distillation and RL also expose a weight-update selector (full, partial, LoRA or QLoRA). Every new field is saved, shared and exported, and participates in exact reviewed-configuration matching. Changes to recipe inputs clear stale performance and memory evidence. Inference task options are unchanged.

- **Full / partial SFT:** BF16 base weights plus 16 bytes of training state per trainable parameter (18 bytes total for full tuning). Partial tuning uses an explicit trainable percentage. Smaller trainable fractions do not automatically imply faster training.
- **Continued pretraining:** corpus tokens × passes; selected weight-update method determines policy memory.
- **DPO:** both prompt-answer sequences count toward training tokens. A resident BF16 reference is scored each epoch, or scores can be precomputed once per run. The reference-precompute stage is sequential; peak memory is the larger stage. An extra runtime allowance covers scoring buffers.
- **Distillation:** offline teacher-generated text followed by student training. A local BF16 teacher generates once per run, sequentially on the same system; peak memory is the maximum of teacher and student stages. Effective output-token throughput must include prefill. External teachers use an explicit dollar cost per run, excluding remote preparation latency from local GPU hours. Already prepared datasets exclude sunk teacher costs. Online/logits distillation is outside this recipe.
- **PPO / GRPO:** prompts × rollout passes × responses × (prompt + completion tokens) × policy-update passes. Generation and reference/reward scoring have separate throughput inputs. PPO adds a full-size, fully trained critic; policy update throughput must include critic updates. GRPO has no critic and requires at least two responses per prompt. Reference scoring is conservatively repeated per update pass; no score-cache speedup is assumed. Optional BF16 rollout/reference copies, reward-model weights, and a runtime/KV-cache allowance add resident memory. External verifier time and per-response fees are explicit inputs. Reward-model training itself is a separate project and excluded.

Additional stages run sequentially on the selected whole system; GPU time includes allocated idle devices during those stages. Total time is (update time + auxiliary stages) × overhead factor + loading/export time. No inter-stage overlap, cross-system services, CPU offload or automatic throughput scaling is assumed. Generic activation/runtime allowances are not architecture-specific memory predictions. No newly added method creates a verified hardware record: the existing hardware/cost verification gate remains in force.

Sources: [training memory anatomy](https://huggingface.co/docs/transformers/model_memory_anatomy), [DPO](https://huggingface.co/docs/trl/dpo_trainer), [sequence-level distillation](https://arxiv.org/abs/1606.07947), [PPO](https://huggingface.co/docs/trl/main/ppo_trainer), [GRPO](https://huggingface.co/docs/trl/grpo_trainer).
