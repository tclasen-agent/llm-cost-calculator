...
test('decision ranking and savings use exactly the displayed horizon totals',()=>{
 ...
 assert.equal(d.winner.key,'api');assert.equal(d.saving,r.last.buy-r.last.api);
 assert.ok(longViewHTML(r).includes('No sustained break-even'));
});
...
test('timeline inspector includes upfront cost at month zero',()=>{
 ...
 const svg=longViewSVG(r,350,0);assert.ok(svg.includes('Assumed life'));assert.ok(svg.includes('Cumulative cost of buying'));assert.ok(!svg.includes('NaN'));
});
test('overview exposes normalized costs and planning exclusions',()=>{
 const r=calculate(planning(defaults)),html=longViewHTML(r);
 for(const text of ['Upfront','Monthly operations','Amortized all-in','No API spillover','No replacement purchases','hardware life'])assert.ok(html.includes(text));
});
